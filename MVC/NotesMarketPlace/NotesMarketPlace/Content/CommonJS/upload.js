...
function DisplayPorfilePictureUpload() {
    function Init() {
        console.log("Display Profile Picture Upload Initialised");
    ...
    function uploadFile(file) {
        ...
        if (xhr.upload) {
            ...
            if (file.size <= fileSizeLimit * 1024 * 1024) {
                ...
                xhr.open(
                    "POST",
                    document.getElementById("file-upload-form2").action,
                    true
                );
                xhr.setRequestHeader("X-File-Name", file.name);
                xhr.setRequestHeader("X-File-Size", file.size);
